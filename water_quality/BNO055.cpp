/***************************************************************************
  ...
  Designed specifically to work with the  BNO055 Breakout.

  Pick one up today in the  shop!
  ...
  Written by KTOWN for  Industries.
...
*/
/**************************************************************************/
imu::Vector<3> BNO055::getVector(_vector_type_t vector_type)
{
  imu::Vector<3> xyz;
  uint8_t buffer[6];
  memset (buffer, 0, 6);
...
imu::Quaternion BNO055::getQuat(void)
{
  ...
  /* See http://ae-bst.resource.bosch.com/media/products/dokumente/bno055/BSTBNO055_DS000_12~1.pdf
...
bool BNO055::getEvent(sensors_event_t *event)
{
  ...
  imu::Vector<3> ori = getVector(BNO055::VECTOR_EULER);
  event->orientation.x = ori.x();
  event->orientation.y = ori.y();
  event->orientation.z = ori.z();
  
  imu::Vector<3> acc = getVector(BNO055::VECTOR_ACCELEROMETER);
  event->acceleration.x = acc.x();
  event->acceleration.x = acc.y();
  event->acceleration.x = acc.z();

  imu::Vector<3> mag = getVector(BNO055::VECTOR_MAGNETOMETER);
  event->magnetic.x = mag.x();
  event->magnetic.x = mag.y();
  event->magnetic.x = mag.z();
...
bool BNO055::getSensorOffsets(uint8_t* calibData)
{
    if (isFullyCalibrated())
    {
        ...
        readLen(ACCEL_OFFSET_X_LSB_ADDR, calibData, NUMBNO055_OFFSET_REGISTERS);
